...
for sample in ["train", "test"]:
    ...
    pu["CluTk"] = elliptic_match(
        pu.CryClu, pu.Tk, etaphi_vars=[["eta", "phi"], ["caloEta", "caloPhi"]], ellipse=ellipse
    )
    ...
    signal["CluTk", "CryCluGenMatch", "CryClu", "id"] = (
        np.round(signal.CryCluGenMatch.CryClu.phi + 3.15, 2) * 100
        + np.round(signal.CryCluGenMatch.CryClu.eta, 2) * 100000
    )
    ...
    def SF(pu_pt, sig_pt):
        ...
        def closure(func, pu_pt, sig_pt):
            ...
            ax.hist(
                ...
                linewidth=2,
                bins=bins,
                weights=np.ones(len(sig_pt)) / sum_sig,
            )
    ...
    sig_dict = {
        feature.split("CryCluGenMatch_")[-1]: np.array(
            ak.flatten(ak.flatten(ak.drop_none(signal.CluTk[*feature.split("_")]), axis=2))
        )
        for feature in sig_features
    }

    bkg_dict = {
        feature: np.array(ak.flatten(ak.flatten(ak.drop_none(pu.CluTk[*feature.split("_")]), axis=2)))
