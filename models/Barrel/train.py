...
df_test, dtest, pca = utils.load_parquet(
    test_file, features, scaler=scaler, ptkey="CC_pt", label2=2 if classes > 2 else 1, pca=pca
)
...
ax2, _ = plots.plot_best_pt_roc(
    df_test, ptkey="CC_pt", thrs_to_select=[0.85, 0.7, 0.35, 0.3, 0.55, 0.85], pt_bins=(0, 5, 10, 20, 30, 50, 150)
)
