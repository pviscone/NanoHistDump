...
model_name="flat3class_131Xv3_xgb1.json"
lightmodel_name="flat3class_131Xv3_light.json"

build=False


xgbmodel=load_xgb_model(model_name)
xgbmodel_light=load_xgb_model(lightmodel_name)
...
ddata_light=xgb.DMatrix(data[features_light], label=data["label"], weight=data["weight"],enable_categorical=True)
...
model_light,data["LightXGBScore"],data["LightHLSScore"]=convert_and_evaluate(xgbmodel_light,ddata_light,light_cfg,"Light")

#%%
#!----------------------Full model----------------------!#

model,data["FullXGBScore"],data["FullHLSScore"]=convert_and_evaluate(xgbmodel,ddata,cfg,"Full")
